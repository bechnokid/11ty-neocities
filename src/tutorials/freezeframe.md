...
If you intend on using the script in multiple HTML files, then you can put `new Freezeframe();` in a separate JS file. However, you will need to make sure the script will run *after* the entire page has been loaded.

For example, the following code would likely go into your `index.js` file.
...
For whatever reason, Freezeframe.js likes to resize `<a>` elements so that any 88x31 button that links to another site is 32 pixels. For this issue, I would suggest just styling the CSS as needed.
